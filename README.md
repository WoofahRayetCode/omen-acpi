...
- Limine with `limine-entry-tool` and at least one primary `linux-cachyos` or
  `linux-cachyos-lts` entry generated by the CachyOS mkinitcpio integration.
...
- Enough ESP space for one small ACPI early CPIO per installed variant, plus
  the independent preventive stock-recovery copies.
...
variant; and creates a separate Limine entry for each detected supported
kernel. It never silently selects a
...
Status reports the running kernel, detected standard/LTS kernels, fallback and
NVIDIA-module observations, and the S5/Combined entry state for each kernel.
It also reports relevant Combined boot messages.
...
| Entry | Purpose |
| --- | --- |
| `linux-cachyos` | unchanged stock standard kernel entry |
| `linux-cachyos-lts` | unchanged stock LTS entry, when installed |
| `linux-cachyos-fallback` / `linux-cachyos-lts-fallback` | unchanged stock fallbacks, when generated; detected and reported but never cloned into experimental entries |
...
Stock entries are never replaced, reordered, hidden or made non-default.
...
Both per-kernel entries reuse the exact stock kernel and ordered stock
`module_path` values. One small, hash-bound ACPI early CPIO is prepended for the
selected variant; kernel and initramfs images are not copied per variant.

## Preventive stock recovery

Before installing a variant, the toolkit prepares a recovery snapshot from one
unambiguous, usable standard or LTS entry during a clean stock boot.
...
`recover-stock` changes nothing on an already clean stock boot. If any usable
supported stock entry still exists, it preserves every stock entry and can offer an explicitly
...
installed documentation, then attempts a transactional migration of recognized
v2.2.0 single-kernel managed state.
...
After installing, updating or removing either supported kernel, reconcile the
owned entries and their Limine BLAKE2 references:

```bash
omen-acpi refresh all
```

Refresh is idempotent: it adds newly installed LTS entries, updates changed
standard/LTS references and removes only obsolete owned entries. Existing
single-kernel managed state is migrated without rebuilding AML. A legacy entry
is reported as `LEGACY / REINSTALL`. Modified or mixed state is
reported as `CONFLICT / BLOCKED` and is never changed automatically. Refresh
also refuses to reuse managed AML after a machine or BIOS identity change.
